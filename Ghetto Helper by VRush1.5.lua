...
function imgui.OnDrawFrame()
    if window.v then
        ...
        if menu == 1 then
            ...
            imgui.Text(u8'íà ñåðûé òåêñò (?) âîçëå ïåðåêëþ÷àòåëÿ ôóíêöèè')
        ...
        if menu == 2 then
            ...
            if imgui.BeginPopup(u8'Ñáèâ íà Z') then
                imgui.Text(u8'Ïðè íàæàòèè íà Z â ÷àò áóäåò îòïðàâëÿòüñÿ ñîîáùåíèå, ñáèâàÿ àíèìàöèþ')
                imgui.EndPopup()
            end
        ...
        if menu == 4 then
            ...
            imgui.Text(u8'/mb') 
            imgui.SameLine()
            imgui.Ques(u8'Áûñòðîå îòêðûòèå /members')
        ...
        sampRegisterChatCommand("na", function(arg)
            if cfg.config.Nabor then
                lua_thread.create(function()
                    ...
                    sampSendChat('/vr Ïðîõîäèò íàáîð â '..bands[g].. ' Âñåõ æäåì íà ðåñïå!')
                    wait(2000)
    ...
    if not reload then
        if Script == thisScript() then
            lua_thread.create(function()
                ...
                msg('Åãî ÂÊ - https://vk.com/write453639513')
            end)
        end
    end
end
